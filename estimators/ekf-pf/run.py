from const import EstimatorConstant, SEED, NOISE_TYPE
...
def evaluate(
    ...
    if ekf_implimented:
        ...
        print(f"length {estimates_ekf[4, -1]}, actual {simulator.states[4, -1]}")
...
def plot(
    ...
    tpf = np.tile(np.arange(0, simulator.constant.N), (num_show, 1)).T.reshape(-1)
    ...
    for i in range(4):
        ...
        if pf_implimented:
            plt.scatter(
                tpf,
                estimates_pf[:, i, :num_show].reshape(1, -1),
                label="PF",
                alpha=0.1,
                c="b",
                s=1,
    ...
    if pf_implimented:
        plt.scatter(
            tpf,
            estimates_pf[:, 4, :num_show].reshape(1, -1),
            label="PF",
            alpha=0.1,
            c="b",
            s=1,
...
if __name__ == "__main__":
    ...
    print(states.shape, inputs.shape, measurements.shape)
