...
# @learning Flask SQLAlchemy converts OOP code to SQL Queries, effectively
# like env params
...
def create_user():
    data = request.get_json()  # converts to dictionary
    new_user = User(
        name=data["name"],
        email=data["email"],
...
def get_one_user(user_id):
    ...
    user = all_users_ll.head
    print(user.data["id"])
    while user:
        if user.data["id"] is int(user_id):
            break
        user = user.next_node
    if user is None:
        return jsonify({"found": False, "message": "User Not Found"}), 400
...
def get_one_blog_posts(blog_post_id):
    ...
    random.shuffle(blog_posts)  # for making the bst possibly more balanced
    ...
    if not post:
        return jsonify({"found": False, "message": "Post not found"}), 400
    return jsonify(post)
...
def delete_last_10():
    ...
    for _ in range(10):
        ...
        removed.append(post_to_delete.data)
        db.session.delete(post_to_delete.data)
        db.session.commit()

    return jsonify({"message": "deleted 10 blogs"})
