...
int main() {
  ...
  while (!exit) {
    ...
    sf::RenderWindow window(sf::VideoMode({800, 600}),
                            ...
                            sf::State::Windowed, contextSettings);
    ...
    glLoadIdentity();
    const GLfloat ratio = static_cast<float>(window.getSize().x) /
    ...
    while (window.isOpen()) {
      ...
      const float x = static_cast<float>(pos.x) * 200.f /
                          static_cast<float>(window.getSize().x) -
                      100.f;
      const float y = -static_cast<float>(pos.y) * 200.f /
                          static_cast<float>(window.getSize().y) +
                      100.f;
